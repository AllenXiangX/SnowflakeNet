...
parser.add_argument('--log_root', type=str, default='/data1/xp/experiments/generation_spdtest/logs_ae/snowflake_cd_only/chair')
...
if args.resume is not None:
    ...
    model = ModelAE(dim_feat=ckpt['args'].latent_dim).to(args.device)
    model.load_state_dict(ckpt['state_dict'])
else:
    model = ModelAE(dim_feat=args.latent_dim).to(args.device)
